...
async def set_cookie(request: CookieSetRequest):
    ...
    try:
        ...
        if not request.cookies.strip():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Cookie字符串不能为空"
            )
        ...
        if success:
            logger.info(
                f"Cookie设置成功 - user_id: {request.user_id}, platform: {request.platform}"
            )
...
async def delete_cookie(user_id: Optional[str] = None, platform: Optional[str] = None):
    """
    删除Cookie
    ...
    **删除逻辑：**
    ...
    try:
        ...
        logger.info(
            f"Cookie删除操作 - user_id: {user_id}, platform: {platform}, 删除数量: {deleted_count}"
        )
...
async def get_cookie_status(
    ...
    **查询逻辑：**
    ...
    try:
        ...
        logger.info(
            f"Cookie状态查询成功 - user_id: {user_id}, platform: {platform}, 总数: {total_count}"
        )

        return CookieStatusResponse(cookies=filtered_cookies, total_count=total_count)
...
async def health_check():
    try:
        ...
        return HealthCheckResponse(
            service="cookie_manager", status="healthy", total_cookies=total_count
        )
    except Exception as e:
