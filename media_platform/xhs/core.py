...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        if config.CRAWLER_MAX_NOTES_COUNT != 0 and config.CRAWLER_MAX_NOTES_COUNT < xhs_limit_count:
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while config.CRAWLER_MAX_NOTES_COUNT == 0 or (page - start_page + 1) * xhs_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                if page < start_page:
                    utils.logger.info(
                ...
                try:
                    ...
                    notes_res = await self.xhs_client.get_note_by_keyword(
                        ...
                        search_id=search_id,
                        page=page,
                        sort=(SearchSortType(config.SORT_TYPE)
                              if config.SORT_TYPE != "" else SearchSortType.GENERAL),
                    )
                    utils.logger.info(
                    ...
                    # Sleep after each page navigation with random delay for anti-detection
                    sleep_time = random.uniform(
                        config.CRAWLER_SLEEP_RANDOM_RANGE[0], config.CRAWLER_SLEEP_RANDOM_RANGE[1]) if config.CRAWLER_SLEEP_RANDOM_RANGE else config.CRAWLER_MAX_SLEEP_SEC
                    await asyncio.sleep(sleep_time)
                    utils.logger.info(
                        f"[XiaoHongShuCrawler.search] Sleeping for {sleep_time:.2f} seconds after page {page-1}")
    ...
    async def get_creators_and_notes(self) -> None:
        ...
        for creator_url in config.XHS_CREATOR_ID_LIST:
            try:
                ...
                creator_info: CreatorUrlInfo = parse_creator_info_from_url(
                    creator_url)
                utils.logger.info(
            ...
            # Use random crawling interval for anti-detection
                ...
                if not note_detail:
                    ...
                    if not note_detail:
                        utils.logger.warning(
                            f"[get_note_detail_async_task] Failed to get note detail, skipping note Id: {note_id}")
                        return None

                note_detail.update(
                    {"xsec_token": xsec_token, "xsec_source": xsec_source})

                # Sleep after fetching note detail with random delay for anti-detection
                sleep_time = random.uniform(
                    config.CRAWLER_SLEEP_RANDOM_RANGE[0], config.CRAWLER_SLEEP_RANDOM_RANGE[1]) if config.CRAWLER_SLEEP_RANDOM_RANGE else config.CRAWLER_MAX_SLEEP_SEC
                await asyncio.sleep(sleep_time)
                utils.logger.info(
                    f"[get_note_detail_async_task] Sleeping for {sleep_time:.2f} seconds after fetching note {note_id}")
    ...
    async def get_comments(self, note_id: str, xsec_token: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            ...
            # Sleep after fetching comments with random delay
            sleep_time = random.uniform(
                config.CRAWLER_SLEEP_RANDOM_RANGE[0], config.CRAWLER_SLEEP_RANDOM_RANGE[1]) if config.CRAWLER_SLEEP_RANDOM_RANGE else config.CRAWLER_MAX_SLEEP_SEC
            await asyncio.sleep(sleep_time)
            utils.logger.info(
                f"[XiaoHongShuCrawler.get_comments] Sleeping for {sleep_time:.2f} seconds after fetching comments for note {note_id}")
    ...
    async def launch_browser(
        ...
        if config.SAVE_LOGIN_STATE:
            ...
            ), "browser_data", config.USER_DATA_DIR % config.PLATFORM)  # type: ignore
    ...
    async def launch_browser_with_cdp(
        ...
        try:
            ...
            utils.logger.info(
                f"[XiaoHongShuCrawler] CDP browser info: {browser_info}")
